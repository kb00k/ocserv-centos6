...
# Detect whether it is the root user
...
#Check if it is CentOS 6 or RHEL 6
...
function ConfigEnvironmentVariable {
    ...
    maxclients=1024
    # The server's certificate and key file are placed in the same directory of the script. The key file's permission should be 600 or 400.

    servercert=${2-server-cert.pem}
    serverkey=${3-server-key.pem}
    # Configuration directory, you can change it to something like /etc/ocserv
    confdir="/usr/local/etc/ocserv"

    #Install system components
    yum install -y -q net-tools bind-utils
	
    # Obtain the interface name of the NIC
...
function CompileOcserv {
    ...
    # Install ocserv dependency components
	...
	cd ..
    #Download ocserv and compile and install
    ...
    # Copy the configuration file sample
    mkdir -p "${confdir}"
...
function ConfigOcserv {
    # Detect whether there is a certificate and key file
    if [[ ! -f "${servercert}" ]] || [[ ! -f "${serverkey}" ]]; then
        # Create ca certificate and server certificate（reference http://www.infradead.org/ocserv/manual.html#heading5）
...
function InstallIBSng {
	...
	sed -i "s/127.0.0.1/${ipv4}/g" /etc/raddb/server
	sed -i "s/other-server/#other-server/g" /etc/raddb/server
...
function PrintResult {
    #Check whether the firewall and ocserv service are normal
    clear
